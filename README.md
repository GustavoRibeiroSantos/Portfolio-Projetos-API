...
##  Em 2019-2 Fatec:
No primeiro semestre, a proposta de projeto para o API foi desenvolvimento de um Webbot que resolvesse algum problema do cotidiano. Assim, pensamos: "Porque não um Webbot faça uma raspagem de dados trazendo preços de cervejas?", após detectar essa brecha no mercado, analisamos com cuidado a problemática. Percebemos que cerveja era algo muito específico e que não teríamos fontes suficientes para extrair os dados necessários. Levando esse fator em consideração, decidimos abranger o leque de opções para bebidas no geral. O Webbot terá como publico alvo, consumidores que gostam de apreciar cerveja ou qualquer bebida especifica. A função do Webbot nada mais é do que coletar inputs dos usuários, usar esses inputs para fazer uma pesquisa na web, e posteriormente, coletar os resultados e exibi-los para o usuário.
...
# Contribuições individuais/pessoais
Para essa aplicação, como a ideia era focar especificamente em uma API back-end. Minha contribuições foram para desenvolver as rotas de web-server para pesquisa, cadastro e atualização dos curriculos, atualização das lista de curriculos aplicados a uma vaga, desenvolvimento e implementação o Observer na aplicação.
